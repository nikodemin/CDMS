...
object Implicits {
  ...
  implicit def conditionalToErrorSeq(conditional: Conditional): Seq[Error] = conditional.errors

  implicit def errorSeqToConditional(errors: Seq[Error]): Conditional = Conditional(errors)

  implicit class StringValidations[S](field: Field[S, String]) {
    def validateLength(length: Int)(implicit source: S): Seq[Error] = field.validate(
      f => if (f.length > length) Some(s"${field.name} is too long") else None
    )

    def validateOnParseError(function: String => Any, formatName: String)(implicit source: S): Seq[Error] = field.validate(
      value => Try(function(value)).toEither.fold(_ => Some(s"${field.name} cannot be parsed as $formatName"), _ => None)
    )

    def validateUUID(implicit source: S): Seq[Error] = validateOnParseError(UUID.fromString, "UUID")
  }
  ...
  implicit class StringSeqValidations[S](field: Field[S, Seq[String]]) {
    def validateElementsLength(length: Int)(implicit source: S): Seq[Error] = field.validateSeq(
      f => f.map(f2 => if (f2.length > length) Some(s"${field.name} is too long") else None)
        .filter(_.isDefined).map(_.get)
    )

    def validateElementsOnParseError(function: String => Any, formatName: String)(implicit source: S): Seq[Error] = field.validateSeq(
      f => f.zipWithIndex.map(fi => Try(function(fi._1)).toEither.fold(_ => Some(s"${field.name}[${fi._2}] cannot be parsed as $formatName"), _ => None))
        .filter(_.isDefined).map(_.get)
    )

    def validateUUIDElements(implicit source: S): Seq[Error] = validateElementsOnParseError(UUID.fromString, "UUID")
  }

  implicit class OptionValidations[S, D](field: Field[S, Option[D]]) {
    def validateRequired(implicit source: S): Seq[Error] = field.validate(
      f => if (f.isEmpty) Some(s"${field.name} is requred") else None
    )

    def validateOneOfOrAbsent(values: D*)(implicit source: S): Seq[Error] = field.validate(
  ...
  implicit class SeqValidations[S, D](field: Field[S, Seq[D]]) {
    def validateNotEmpty(implicit source: S): Seq[Error] = field.validate(
  ...
  implicit class NumberOptionValidations[S, D: Numeric](field: Field[S, Option[D]]) {
    def validatePositive(implicit source: S): Seq[Error] = field.validate(
      f => {
    ...
    def validateRange(min: Int, max: Int)(implicit source: S): Seq[Error] = field.validate(
      f => {
        val value = f.map(implicitly[Numeric[D]].toInt)
  ...
  implicit class CommonValidations[S, D](field: Field[S, D]) {
    def validateOneOf(values: D*)(implicit source: S): Seq[Error] = field.validate(
